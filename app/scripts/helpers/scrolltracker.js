...
(function(window) {
   ...
   */
  window.ScrollTracker = function(opts) {
    ...
    if (typeof context === 'string') context = document.getElementsByTagName('slide-manager')[0];
  ...
  ScrollTracker.prototype._checkDepth = function() {
    ...
    for (key in marks) {
      ...
      if (currentDepth >= key && !this._tracked[key]) {
	    //console.log("passed key: current depth = " + currentDepth + " for key = " + key);
  ...
  ScrollTracker.prototype._contextHeight = function() {
    ...
    if (this._context !== document.body) return this._context.offsetHeight - 5;
  ...
  ScrollTracker.prototype._currentDepth = function() {
    ...
    if (!this._context.scrollTop) {
      ...
      if (!this._context.scrollTop) {
        ...
        depth = (window.scrollY || window.pageYOffset ||
  ...
  function docHeight() {
    ...
    var slides = document.getElementsByTagName("slide-manager")[0];
    
    var docHeight = Math.max(slides.scrollHeight, body.scrollHeight, body.offsetHeight,
  ...
  function visibleInViewport(element) {
    ...
    var vHeight = Math.max(0, rect.top > 0 ? Math.min(height, windowHeight - rect.top) : (rect.bottom < windowHeight ? rect.bottom : windowHeight));
    return vHeight;
  }
...
(function(window) {	    
  if (document.readyState !== 'loading') {
    ...
    document.addEventListener('slidesLayoutComplete', init);
  }
    ...
    tracker.on({
      percentages: {
		...
		gtag('event', evt.data.label, {
			...
			'event_label': window.location.href.split('#')[1]
		});
